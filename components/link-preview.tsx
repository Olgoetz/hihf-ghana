...
async function LinkPreview({ url }: { url: string }) {
  ...
  return (
    <Link
      href={url}
      target="_blank"
      className="text-black mx-auto mt-8  w-[90%] h-full md:h-[200px] cursor-pointer flex items-center bg-[#f3f3f3] gap-3 text-left border-white border-[2px]"
      style={{
        textDecoration: "none",
      }}
    >
      <div className="object-cover h-full">
        <img
          src={data.image}
          alt="Link Preview"
          className="md:object-cover hidden md:flex h-full w-[340px] m-0"
        />
      </div>
      <div className="md:px-4 w-full md:w-[60%]">
        <h3 className="text-lg font-bold leading-[2rem] mb-2 ">{data.title}</h3>
